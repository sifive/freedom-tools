...
FREEDOM_TOOLCHAIN_METAL_COMMIT := 450de694d4fb94d94d45241e3f2d90e8f161aca8
...
toolchain-metal-package: \
		$(SRCPATH_FREEDOM_TOOLCHAIN_METAL).$(FREEDOM_TOOLCHAIN_METAL_COMMIT)
	$(MAKE) -C $(SRCPATH_FREEDOM_TOOLCHAIN_METAL) package POSTFIXPATH=$(abspath .)/ EXTRA_OPTION=$(EXTRA_OPTION) EXTRA_SUFFIX=$(EXTRA_SUFFIX)

toolchain-metal-regress: \
		$(SRCPATH_FREEDOM_TOOLCHAIN_METAL).$(FREEDOM_TOOLCHAIN_METAL_COMMIT)
	$(MAKE) -C $(SRCPATH_FREEDOM_TOOLCHAIN_METAL) regress POSTFIXPATH=$(abspath .)/ EXTRA_OPTION=$(EXTRA_OPTION) EXTRA_SUFFIX=$(EXTRA_SUFFIX)

toolchain-metal-cleanup:
	$(MAKE) -C $(SRCPATH_FREEDOM_TOOLCHAIN_METAL) cleanup POSTFIXPATH=$(abspath .)/
	rm -rf $(SRCPATH_FREEDOM_TOOLCHAIN_METAL).*
	rm -rf $(SRCPATH_FREEDOM_TOOLCHAIN_METAL)

toolchain-metal-flushup:
	$(MAKE) -C $(SRCPATH_FREEDOM_TOOLCHAIN_METAL) flushup POSTFIXPATH=$(abspath .)/
